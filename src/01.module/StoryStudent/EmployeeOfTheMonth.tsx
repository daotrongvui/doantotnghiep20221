...
export class StoryStudentToeic extends BaseComponent<StoryStudentProps, StoryStudentStates> {
  ...
  async getDataStoryStudent() {
    let allData = orderBy(await storyStudentService.getAll<StoryStudent>('StoryStudent'), 'Order', 'asc');
  ...
  public renderItem() {
    ...
    let star = (
      <svg
        width="25"
        height="25"
        className={styles.StoryStudent__contentLeft__content__contaninerMonth__iconStar}
        viewBox="0 0 25 25"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
      >
    ...
    return this.state.allData.map((item) => (
      <div className={styles.StoryStudent}>
        <div className={styles.StoryStudent__contentLeft}>
          <img className={styles.StoryStudent__contentLeft__backgroundImage} src={`${item.BannerHomeUrl}`} />
            ...
            >
              ...
              {165 < item.Title.length ? (
                <Tooltip title={item.Title}>
                  <div className={styles.StoryStudent__contentLeft__content__description}>
                    {' '}
                    {item.Title.slice(0, 164).concat('...')}{' '}
                  </div>
                </Tooltip>
              ) : (
                <div className={styles.StoryStudent__contentLeft__content__description}>{item.Title}</div>
              )}
              <div className={styles.StoryStudent__contentLeft__content__contaninerMonth}>
                {star}
                <div className={styles.StoryStudent__contentLeft__content__contaninerMonth__dateAndMonth}>
        ...
        <div className={styles.StoryStudent__contentRight}>
          <div className={styles.StoryStudent__contentRight__ViewContent}>
            <div className={styles.StoryStudent__contentRight__ViewContent__title}>Chuyện học hành</div>
            <Link to={`${ROUTER.STORYSTUNDENTDETAIL}?keyDoc=${item.KeyDoc}`}>
              <div className={styles.StoryStudent__contentRight__ViewContent__inforUser}>
                <img
                  src={`${item.Avatar}`}
                  className={styles.StoryStudent__contentRight__ViewContent__inforUser__avartar}
                />
                <div className={styles.StoryStudent__contentRight__ViewContent__inforUser__infor}>
                  <div className={styles.StoryStudent__contentRight__ViewContent__inforUser__infor__name}>
                    {item.Name}
                  </div>
                  <div className={styles.StoryStudent__contentRight__ViewContent__inforUser__infor__jobTitle}>
                    {item.JobTitle ? item.JobTitle : ''}
                  </div>
                </div>
              </div>
            </Link>
            <div className={styles.StoryStudent__contentRight__ViewContent__decription}>{item.Description1}</div>
          </div>
        </div>
      </div>
    ));
