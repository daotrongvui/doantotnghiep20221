...
export default class ThemeDetailModalCom extends BaseComponent<ThemeDetailModalProps, ThemeDetailModalState> {
  ...
  async saveNoteWord(item: ContentItem) {
    ...
    try {
      if (this.state.myNoteWord && this.state.myNoteWord.Content.length > 0) {
        ...
        await myNoteWordService.update<MyNoteWord>('MyNoteWord', this.props.currentUser.Uid, newData);
      } else {
        await myNoteWordService.save<MyNoteWord>('MyNoteWord', this.props.currentUser.Uid, {
      ...
      >
        <Spin spinning={this.state.loading}>
          <div className={styles.themeDetail}>
            <div className={styles.themeDetail__titleIntroduce}>Bài 1: Contracts - Hợp Đồng</div>
              ...
              (this.state.item.Content as any[]).map((item: ContentItem, index) => (
                <div className={styles.themeDetail__item}>
                  <div style={{ display: 'flex', alignItems: 'center' }}>
                    <div className={styles.themeDetail__item__index}>{index + 1}</div>
                    <img className={styles.themeDetail__item__img} src={item.ImgItem} />
                    <div className={styles.themeDetail__item__infor}>
                      <div>
                        <span className={styles.themeDetail__item__infor__title}>{item.Title}:</span>
                        <span className={styles.themeDetail__item__infor__spelling}>{item.Spelling}</span>
                      </div>
                      <div>
                        <span className={styles.themeDetail__item__infor__bold}>Giải thích:</span>
                        <span>{item.Example}</span>
                      </div>
                      <div>
                        <span className={styles.themeDetail__item__infor__bold}>Từ loại:</span>
                        <span>{item.Category}</span>
                      </div>
                      <div>
                        <span className={styles.themeDetail__item__infor__bold}>Ví dụ:</span>
                        <span>{item.Explain}</span>
                      </div>
                      <div> 
                        <span className={styles.themeDetail__item__infor__bold}>{item.Translate}</span>
                      </div>
