...
import { faUser, faClock, faCalendar, faQuestionCircle, faEye, faEdit } from '@fortawesome/free-solid-svg-icons';
...
export class ListExamPart1 extends BaseComponent<ListExamPart1Props, ListExamPart2State> {
  ...
  async getAllData() {
    let allData = await toeicPart1ExamService.getAll<ToeicPart1Exam>('ToeicPart1Exam', '');
        ...
        <div className={styles.Contanier__leftcontent__item}>
          ...
          <div className={styles.Contanier__leftcontent__item__infor}>
            <div className={styles.Contanier__leftcontent__item__infor__title}>
              Đề luyện Toeic Part 1 - Ets 2020 - {item.Title}- Có đáp án chi tiết
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <div className={styles.Contanier__leftcontent__item__infor__wrapInfor}>
                {/* <div className={styles.Contanier__leftcontent__item__infor__wrapInfor__item}>
                  <span className={styles.examShortExam__content__infor}>
                    <FontAwesomeIcon icon={faUser} style={{ marginRight: 5, color: '#909399' }} />
                    {item.Creator.Title}
                  </span>
                </div> */}
                <div className={styles.Contanier__leftcontent__item__infor__wrapInfor__item}>
                  <FontAwesomeIcon icon={faCalendar} style={{ marginRight: 5, color: '#909399' }} />

                  {moment(new Date(item.Created.seconds * 1000).toUTCString()).format('DD/MM/YYYY')}
                </div>
                <div className={styles.Contanier__leftcontent__item__infor__wrapInfor__item}>
                  <FontAwesomeIcon icon={faClock} style={{ marginRight: 5, color: '#909399' }} />
                  {item.Time / 60} phút
                </div>
                <div className={styles.Contanier__leftcontent__item__infor__wrapInfor__item}>
                  <FontAwesomeIcon icon={faQuestionCircle} style={{ marginRight: 5, color: '#909399' }} />
                  {item.LookUpKeyDoc.length} câu
                </div>
              </div>
              <div className={styles.Contanier__leftcontent__item__infor__viewAndExam}>
                <div
                  style={{ backgroundColor: '#FF9933', color: 'white' }}
                  className={styles.Contanier__leftcontent__item__infor__viewAndExam__item}
                >
                  <FontAwesomeIcon icon={faEdit} style={{ marginRight: 5, color: 'white' }} /> {item.CountItem} lượt thi
                </div>
                <div
                  style={{ backgroundColor: '#00CC00', color: 'white' }}
                  className={styles.Contanier__leftcontent__item__infor__viewAndExam__item}
                >
                  <FontAwesomeIcon icon={faEye} style={{ marginRight: 5, color: 'white' }} /> {item.View} lượt xem
                </div>
              </div>
            </div>
          </div>
      ...
      <div>
        <div className={styles.Contanier}>
          <div className={styles.Contanier__leftcontent}>
            ...
            {this.state.allData.length > 0 && this.state.allData.map((item) => this.renderItem(item))}
          </div>
