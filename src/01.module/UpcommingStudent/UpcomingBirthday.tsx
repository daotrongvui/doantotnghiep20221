...
export class UpcomingBirthdays extends BaseComponent<UpcomingBirthdaysProps, UpcomingBirthdaysStates> {
        ...
        return (
          ...
          >
            {this.state.isHover && this.state.key == index ? (
              <div className={styles.upcomingBirthdaysSlider__slider__itemHover}>
                {item.PhotoUrl ? (
                  <img
                    src={`${item.PhotoUrl}?width=48&height=48`}
                    className={styles.upcomingBirthdaysSlider__slider__itemHover__avartar}
                  />
                ) : item.Sex === 'Male' ? (
                  <img className={styles.upcomingBirthdaysSlider__slider__itemHover__avartar} src={imgUser} />
                ) : (
                  <img className={styles.upcomingBirthdaysSlider__slider__itemHover__avartar} src={imgUser} />
                )}
                <div className={styles.upcomingBirthdaysSlider__slider__itemHover__name}>{item.LoginName}</div>
                <div className={styles.upcomingBirthdaysSlider__slider__itemHover__jobTitle}>{item.JobTitle}</div>
                <div className={styles.upcomingBirthdaysSlider__slider__itemHover__email}>{item.Email}</div>
                <div className={styles.upcomingBirthdaysSlider__slider__itemHover__workPhone}>{item.PhoneNumber}</div>
                <div
                  onClick={() => {
                    window.location.href = `MSTeams:/l/chat/0/0?users=${item?.LoginName}`;
                  }}
                  className={styles.upcomingBirthdaysSlider__slider__itemHover__msTeam}
                >
                  {iconMSTeam}
                  Gửi tin nhắn
                </div>
              ...
              <div className={styles.upcomingBirthdaysSlider__slider__item}>
                <div className={styles.upcomingBirthdaysSlider__slider__item__message}>{_.get(item, 'Wish')}</div>
                <div className={styles.upcomingBirthdaysSlider__slider__item__inforUser}>
                  {item.PhotoUrl ? (
                    <img
                      src={item.PhotoUrl}
                      height="48px"
                      width="48px"
                      className={styles.upcomingBirthdaysSlider__slider__item__inforUser__avartar}
                    />
                  ) : item.Sex === 'Female' ? (
                    <img
                      height="48px"
                  ...
                  <div className={styles.upcomingBirthdaysSlider__slider__item__inforUser__title}>
                    <div className={styles.upcomingBirthdaysSlider__slider__item__inforUser__title__content1}>
                      {item.LoginName}
                    </div>
                    <div className={styles.upcomingBirthdaysSlider__slider__item__inforUser__title__content2}>
                      Sinh viên
                    </div>
                ...
                <div className={styles.upcomingBirthdaysSlider__slider__item__footer}>{iconFooter}</div>

                <div className={styles.upcomingBirthdaysSlider__slider__item__dateOfBirth}>
                  <span className={styles.upcomingBirthdaysSlider__slider__item__dateOfBirth__day}>12</span>
                  <span className={styles.upcomingBirthdaysSlider__slider__item__dateOfBirth__dotted}>.</span>
                  <span className={styles.upcomingBirthdaysSlider__slider__item__dateOfBirth__month}>03</span>
                  {item.DateOfBirth && (
                    <span>
                      <span className={styles.upcomingBirthdaysSlider__slider__item__dateOfBirth__dotted}>.</span>
                      <span className={styles.upcomingBirthdaysSlider__slider__item__dateOfBirth__day}>2022</span>
                    </span>
                  )}
                </div>

                <div className={styles.upcomingBirthdaysSlider__slider__item__elip}>{iconElip}</div>
              </div>
            )}
          </div>
        );
  ...
  public getResponsiveSlider(lengthList: number) {
    const settings = {
      ...
      autoplay: false,
      nextArrow:
        this.state.lastIndex == this.state.dataSource.length ? <SampleNextArrow /> : <SampleNextArrowHighLight />,
      prevArrow: this.state.firstindex == 0 ? <SamplePrevArrow /> : <SamplePrevArrowHighLight />,
      swipe: false,
      ...
      <div className={styles.upcomingBirthdaysSlider}>
        ...
        <Skeleton paragraph={{ rows: 5 }} loading={this.state.loading}>
          {this.state.dataSource && this.state.dataSource.length > 0 ? (
            <Slider {...settings} className={styles.upcomingBirthdaysSlider__slider}>
              {this.renderAllItem()}
            </Slider>
          ) : (
            <Empty
...
const iconMSTeam = (
  <svg
    className={styles.upcomingBirthdaysSlider__slider__itemHover__msTeam__iconMSTeam}
    width="18"
    height="18"
    viewBox="0 0 18 18"
    fill="none"
...
const iconFooter = (
  ...
  >
    ...
    <circle cx="209.917" cy="23.3972" r="1.99472" transform="rotate(173.197 209.917 23.3972)" fill="#FFA940" />
    <circle cx="317.217" cy="21.2166" r="1.99472" transform="rotate(173.197 317.217 21.2166)" fill="#FFA940" />
    <circle cx="130.93" cy="15.5339" r="2.81816" transform="rotate(-60.9464 130.93 15.5339)" fill="#FFD666" />
    <circle cx="247.832" cy="17.8318" r="2.81816" transform="rotate(-60.9464 247.832 17.8318)" fill="#FFD666" />
    <circle cx="12.4591" cy="4.49145" r="3.05395" fill="#FFA940" />
    <circle cx="52.4504" cy="7.76051" r="1.96314" transform="rotate(125.857 52.4504 7.76051)" fill="#FFA940" />
    <circle cx="159.159" cy="11.5627" r="3.05395" fill="#FA8C16" />
    <path d="M370.794 6.99979L371.686 14.4736L364 7.81034L370.794 6.99979Z" fill="#FFBB96" />
    <path d="M70.3041 0.596566L75.4453 0.0693359L70.7834 5.27043L70.3041 0.596566Z" fill="#FFC069" />
    <path d="M6.65307 16.9627L4.06885 21.4385L2.58421 14.6134L6.65307 16.9627Z" fill="#FFC069" />
...
const iconElip = (
  ...
  >
    ...
    <defs>
      <linearGradient id="paint0_linear" x1="141" y1="0" x2="63.0959" y2="191.863" gradientUnits="userSpaceOnUse">
  ...
  return (
    ...
    >
      ...
      <path
        ...
        fill={'#e9ab17'}
      />
      <rect x="0.5" y="0.5" width="23" height="23" rx="3.5" stroke={'#e9ab17'} />
    </svg>
  );
}
  ...
  return (
    ...
    >
      ...
      <path
        ...
        fill={'#BFBFBF'}
      />
      <rect x="0.5" y="0.5" width="23" height="23" rx="3.5" stroke={'#BFBFBF'} />
    </svg>
  );
}
