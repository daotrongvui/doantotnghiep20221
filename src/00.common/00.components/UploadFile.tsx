...
export class UploadFile extends BaseComponent<UploadFileProps, UploadFileState> {
        ...
        return (
            <div>
                <CustomUploadButton
                    ...
                    storageRef={firebase.storage().ref(`CommonDoc/${this.props.refDocLib}`)}
                ...
                >
                    Choose {this.props.type == 'img' ? 'Image' : 'Audio'} <UploadOutlined />
                </CustomUploadButton>
                    ...
                    {this.props.showImg !== false && this.props.type == 'img' && (
                        <div>
                            {this.state.downloadURLs.map((downloadURL, i) => {
                                return <img key={i} src={downloadURL} height={150} width={200} />;
                            })}
                        </div>
                    )}
                    {this.props.type == 'audio' && (
                        <div>
                            {this.state.downloadURLs.length > 0 &&
                                this.state.downloadURLs.map((downloadURL, i) => {
                                    return <ReactAudioPlayer key={i} src={downloadURL} autoPlay={false} controls />;
                                })}
                        </div>
                    )}
                </div>
