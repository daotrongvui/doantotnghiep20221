...
import { SearchOutlined, PlusCircleOutlined, TableOutlined, IdcardOutlined } from '@ant-design/icons';
...
const { Option } = Select;
export default class List600WordsToeic extends BaseComponent<List600WordProps, List600WordState> {
    ...
    async loadAllData() {
        let allData = _.orderBy(await words600Service.getAll<WordsToeic>('600WordsToeic'), 'OrderBy', 'asc');

        let allCategories: { Title: string; value: string }[] = allData.map((item) => {
            return {
                Title: item.Title,
                value: item.KeyDoc,
            };
    ...
    async onSelect(value: any, isRefresh: boolean) {
        let allData: any[] = [];
        if (isRefresh) {
            allData = _.orderBy(await words600Service.getAll<WordsToeic>('600WordsToeic'), 'OrderBy', 'asc');
        } else {
        ...
        await this.setState({
            dataSource: dataSelect.Content && dataSelect.Content.length > 0 ? dataSelect.Content : [],
    ...
    render() {
        const columns = [
            ...
            },
            {
                ...
                render: (LinkAudio: any) => <ReactAudioPlayer src={LinkAudio} autoPlay={false} controls />,
            },
            ...
            <div>
                ...
                >
                    ...
                    >
                        ...
                        <IdcardOutlined
                            ...
                            style={{ fontSize: 25, color: '#e9ab17', marginRight: 10 }}
                        />
                        <Select
                ...
                {this.state.viewTable ? (
                    <Table
                        ...
                        dataSource={
                            this.state.dataSource && this.state.dataSource.length > 0 ? this.state.dataSource : []
                        }
                    />
                ) : (
